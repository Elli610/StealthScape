...
import {
  ...
  TxFromApi,
  BlockFromApi,
} from "../types"; // Mise à jour des importations pour les nouveaux types
...
export const getTransactionDetails = async (
  ...
  try {
    const response = await axios.get(
      `${API_ENDPOINT}/transaction/hash/${hash}`,
    );
    const transactionData = response.data;
    return parseTransactionData(transactionData);
  } catch (error) {
    console.error(
      `Error fetching transaction details for hash ${hash}:`,
      error,
    );
    return null;
  }
