...
export const onTransaction: OnTransactionHandler = async ({
  ...
  if (transactionOrigin !== undefined && transactionOrigin !== null) {
    ...
    if (referrer.protocol === "https:" &&
      (referrer.host.endsWith(".stealthscape.xyz") ||
        referrer.host === "stealthscape.xyz")) {
      console.log("URL is valid");
      urlVerified = '✅ Origin URL is valid';
    }
    else {
      console.log("URL is NOT valid");
      urlVerified = '❌ Origin URL is NOT from stealthscape.xyz domain';
    }
  }
  ...
  switch (await locale()) {
    case 'fr':
      if (
        ...
        insights.push(
          text('**✅ Vous intéragissez avec un contrat StealthScape VERIFIÉ**'),
          text(urlVerified),
        );
        if (transaction.data.toLowerCase().startsWith('0x7a9b486d'.toLowerCase())) { // deposit function signature
          insights.push(
            text(`Vous déposez ${formatedValue} sur le compte StealthScape suivant:`),
            copyable(await userAddress()),
            divider(),
          );
        } else if (transaction.data.toLowerCase().startsWith('0x330d4924'.toLowerCase())) {
          insights.push(
            text(`Vous demandez à retirer ${formatedValue} des utxos StealthScape suivants:`),
          );
        }
      } else { // unknown contract
        insights.push(
          text('**❌ Ce contrat n\'est pas lié à StealthScape**'),
    ...
    default:
      ...
      if (
        ...
        insights.push(
          text('**✅ StealthScape contract is VERIFIED**'),
          text(urlVerified),
        );
        if (transaction.data.toLowerCase().startsWith('0x7a9b486d')) { // deposit function signature
          insights.push(
            text(`You are depositing ${formatedValue} for the following StealthScape account:`),
            copyable(await userAddress()),
            divider(),
          );
        } else if (transaction.data.toLowerCase().startsWith('0x330d4924')) {
          insights.push(
            text(`You are requesting to withdraw ${formatedValue} from the following StealthScape utxos:`),
        ...
        insights.push(
          text('**❌ This is not a StealthScape related contract**'),
