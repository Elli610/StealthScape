use crate::database::insert_transaction;
use crate::interface;
use crate::interface::{
    ...
    for tx in pending_txs {
        match tx {
            ...
            PendingTransaction::PendingDeposit(deposit_tx) => {
                ...
                let finalized_transaction = Transaction::from_user_deposit_tx(
            ...
            PendingTransaction::PendingRingCTx(ring_ct) => {
                ...
                for output in ring_ct.outputs.clone() {
                    let _ = insert_utxo(interface::utxo::utxo::UTXO::Payment(output.clone()));
                }
                // then add the tx to the transaction database, and to the vector of finalized txs
                let finalized_transaction =
                    Transaction::from_ringCTx(ringCTx::from_pending_ringCTx(ring_ct.clone()));
                let _ = insert_transaction(finalized_transaction.clone());

                finalized_txs.push(finalized_transaction);
                //then delete the tx from the mempool
                let _ = delete_transaction_from_mempool(ring_ct.clone().hash);

                // add the hash to the hashes vector
                hashes.push(ring_ct.hash);
            }
        }
    }
