...
import { TxToVerify, UnsignedPaymentTX } from "../interfaces";
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    if (body.inputs.length >= 1 && body.outputs.length >= 1) { // excludes coinbase and exit transactions
      let sumInputs: Point = Point.deserialize(body.inputs[0]);
      for (let i = 1; i < body.inputs.length; i++) {
        sumInputs = sumInputs.add(Point.deserialize(body.inputs[i]));
      }

      let sumOutputs: Point = Point.deserialize(body.outputs[0].commitment);
      for (let i = 1; i < body.outputs.length; i++) {
        sumOutputs = sumOutputs.add(Point.deserialize(body.outputs[i].commitment));
      }
      console.log("sumInputs: ", sumInputs.serialize());
